...
describe("getAdminUserToken", () => {
  ...
  it("環境変数が設定されていない場合はエラーを投げる", () => {
    ...
    try {
      ...
      assertStringIncludes(message, "token");
    }
  });
});

describe("setStatusWithUserToken", () => {
  const originalToken = Deno.env.get("SLACK_ADMIN_USER_TOKEN");
  ...
  it("正常にステータスを設定できる", async () => {
    ...
    globalThis.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      assertStringIncludes(headers["Authorization"], "Bearer xoxp-test-token");
...
describe("clearStatusWithUserToken", () => {
  const originalToken = Deno.env.get("SLACK_ADMIN_USER_TOKEN");
  let originalFetch: typeof globalThis.fetch;
